...
# =======================================================
# Step 2a: Prepare raw featuremap stats
# =======================================================
...
# =======================================================
# Step 3a: Clean FP VCF to remove non ACGT flanking bases in the INFO field
# =======================================================
#Question for u Tobi - should I use the raw featuremap or the raw training regions

VCF_IN_FP="$OUTPUT_DIR/${BASE}.raw.training_regions.vcf.gz"
VCF_OUT_FP="$OUTPUT_DIR/${BASE}.raw.training_regions.clean.vcf.gz"
BAD_POSITIONS_FP="$OUTPUT_DIR/bad_positions_FP.txt"
...
bcftools query -f '%CHROM\t%POS\t[%INFO/X_NEXT1]\t[%INFO/X_PREV1]\t[%INFO/X_NEXT2]\t[%INFO/X_PREV2]\t[%INFO/X_NEXT3]\t[%INFO/X_PREV3]\n' "$VCF_IN_FP" \
  ...
  > "$BAD_POSITIONS_FP"
echo "Found $(wc -l < $BAD_POSITIONS_FP) bad positions"

if [ ! -s "$BAD_POSITIONS_FP" ]; then
  echo "No bad positions found, copying input VCF as clean version"
  cp "$VCF_IN_FP" "$VCF_OUT_FP"
  cp "$VCF_IN_FP.tbi" "$VCF_OUT_FP.tbi"
else
  echo "Filtering out $(wc -l < $BAD_POSITIONS_FP) bad positions"
  bcftools view -T ^"$BAD_POSITIONS_FP" "$VCF_IN_FP" -Oz -o "$VCF_OUT_FP"
  bcftools index -t "$VCF_OUT_FP"
fi
...
echo "Cleaned VCF written to $VCF_OUT_FP"

# =========================
# Step 3b: Prepare RANDOM SAMPLE (FP) for parquet and filtering
# =========================
...
singularity exec --bind $BIND_PATHS $UGBIO_FEATUREMAP_DOCKER filter_featuremap \
  ...
  --filter name=no_adj_ref_diff:field=ADJ_REF_DIFF:op=eq:value=0:type=quality \
  --filter name=bcsq_gt_40:field=BCSQ:op=gt:value=40:type=quality \
  --filter name=edist_le_10:field=EDIST:op=lt:vafvascf aaue=10:type=quality \
  --filter name=alt_hmer_lt_7:field=X_HMER_ALT:op=lt:value=7:type=quality \
  --filter name=vaf_le_threshold:field=RAW_VAF:op=le:valude=${MAX_VAF_FOR_FP}:type=label \
  --downsample random:${FP_TRAIN_SET_SIZE}:${RANDOM_SEED}

# =========================
# iStep 4: Prepare RANDOM SAMPLE (TP)
# =========================
...
# =======================================================
# Step 4a: Clean TP VCF to remove non ACGT flanking bases in the INFO field
# =======================================================

VCF_IN_TP="$OUTPUT_DIR/${BASE}.rs.training_regions.vcf.gz"
VCF_OUT_TP="$OUTPUT_DIR/${BASE}.rs.training_regions.clean.vcf.gz"
BAD_POSITIONS_TP="$OUTPUT_DIR/bad_positions_TP.txt"

echo "Step 1: Finding bad variants with non-ACGT flanking INFO fields"
bcftools query -f '%CHROM\t%POS\t[%INFO/X_NEXT1]\t[%INFO/X_PREV1]\t[%INFO/X_NEXT2]\t[%INFO/X_PREV2]\t[%INFO/X_NEXT3]\t[%INFO/X_PREV3]\n' "$VCF_IN_TP" \
  | awk '{for (i=3;i<=8;i++) if ($i !~ /^[ACGT]$/) {print $1"\t"$2; break}}' \
  > "$BAD_POSITIONS_TP"
echo "Found $(wc -l < $BAD_POSITIONS_TP) bad positions"
...
#echo "Job finished at: $(date)"
#echo "Cleaned VCF written to $VCF_OUT_TP"


# =========================
# Step 4b: Prepare RANDOM SAMPLE (TP) for parquet and filtering
# =========================
...
singularity exec --bind $BIND_PATHS $UGBIO_FEATUREMAP_DOCKER filter_featuremap \
  ...
  #try setting ref ne to alt instead of eq
...
singularity exec \
  --bind /cluster/home/t922316uhn/singularity/srsnv_training.py:/opt/ugbio/srsnv_training.py,$BIND_PATHS \
  "$UGBIO_SRSNV_DOCKER" \
