...
singularity exec --bind $BIND_PATHS $FEATUREMAP_DOCKER true >/dev/null 2>&1
...
singularity exec --bind $BIND_PATHS $UGBIO_FEATUREMAP_DOCKER srsnv_training \
...
mv -f "${OUTPUT_DIR}/${BASE}.model_fold_"* "${OUTPUT_DIR}/model_files/" 2>/dev/null || true
if [ -f "${OUTPUT_DIR}/${BASE}.srsnv_metadata.json" ]; then
  mv -f "${OUTPUT_DIR}/${BASE}.srsnv_metadata.json" "${OUTPUT_DIR}/model_files/srsnv_metadata.json"
fi

# Run snvqual: input RAW featuremap vcf, output into OUTPUT_DIR
QUAL_VCF=${OUTPUT_DIR}/${BASE}.featuremap.vcf.gz
singularity exec --bind $BIND_PATHS $UGBIO_FEATUREMAP_DOCKER snvqual "$RAW_FM_VCF" "$QUAL_VCF" "${OUTPUT_DIR}/model_files/srsnv_metadata.json" -v
...
singularity exec --bind $BIND_PATHS $UGBIO_FEATUREMAP_DOCKER srsnv_report \
