...
#SBATCH --output=rtg_BMvUltima_%j.out
#SBATCH --error=rtg_BMvUltima_%j.err
...
OUTDIR=/cluster/projects/pughlab/myeloma/projects/MM_cell_drugs/WGS_Pipeline/UGBIODocker/sample/vcfeval_results
mkdir -p "$OUTDIR"

# --- Loop over each sample in the table ---
tail -n +2 samples_Ultima.txt | while read -r SAMPLE BM_ID CF_ID; do
