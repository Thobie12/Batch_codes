...
# === Global summary file ===
SUMMARY_FILE="$OUTDIR/all_samples_intersection_counts.tsv"
...
while read -r SAMPLE; do
    ...
    # Extract patient code (matches CA-08-R-P, HP-05-01-P, VA-09-01-P etc.)
    PATIENT_CODE=$(echo "$SAMPLE" | grep -oE '[A-Z]{2}-[0-9]{2}-[0-9]{2}(-[RP])?-Px')

    # Find the correct Illumina file using patient code
