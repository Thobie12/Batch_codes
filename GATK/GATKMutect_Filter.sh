...
#SBATCH --dependency=afterok:3500261
...
MERGED_DIR="/cluster/projects/pughlab/myeloma/projects/MM_cell_drugs/WGS_Pipeline/GATK/${SAMPLE}/Merged"
OUT_DIR="${MERGED_DIR}/vcf"
mkdir -p "$OUT_DIR"
...
# ==============================
echo "[$(date)] Step 2: Running LearnReadOrientationModel for $SAMPLE"

INPUTS=""
shopt -s nullglob
for f in "${MERGED_DIR}"/*_f1r2.tar.gz; do
    INPUTS+=" -I $f"
done
shopt -u nullglob

if [[ -z "$INPUTS" ]]; then
    echo "ERROR: No f1r2 files found in $MERGED_DIR"
    ls -lh "$MERGED_DIR"
    exit 1
fi
...
# ==============================
echo "[$(date)] Step 3: Running FilterMutectCalls for $SAMPLE"
...
# ==============================
# Step 4: Exclude blacklist + repeats
# ==============================
echo "[$(date)] Step 4: Excluding ENCODE blacklist + RepeatMasker regions"
